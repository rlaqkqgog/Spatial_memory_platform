...
정답지(`spawned_stone_positions.csv` 등 spawned object 로그)와 참가자 제출을 **행 순서대로 1:1 비교하지 않습니다.** 두 자료는 그대로 맞대면 전부 오답이 나기 때문입니다.

- **행 순서가 다름** — 정답지는 `blue_1, blue_2, …` 순, 제출은 `red_1, red_2, …` 순이라 행 번호로 맞대면 서로 다른 돌끼리 비교됩니다.
- **좌표계가 다름** — 정답지는 월드 좌표(미터, `sourceMode=MRUK_ROOM_LOCAL`/`WORLD`), 제출은 평면도 정규화 좌표(`0~1`, `sourceMode=PLAN`)입니다. 단위·공간이 달라 숫자 직접 비교는 무의미합니다.

실제 채점은 [`scoreMarkers`](src/lib/answer-key.ts)에서 다음 순서로 이뤄집니다.

1. **색상별로 묶기** — 빨강·파랑·초록·노랑 각 색상 안에서만 비교합니다. `blue_1`/`blue_2` 같은 개별 번호나 CSV 행 순서는 채점에 쓰지 않습니다.
2. **좌표계 통일(캘리브레이션)** — 참가자의 평면도 정규화 좌표(0~1)를 평면도별 `plan→world` 아핀 변환([`planToWorld`](src/lib/answer-key.ts))으로 월드 좌표(미터)로 바꿔, 월드 좌표인 정답 스톤과 **같은 공간**에서 비교합니다. 이 변환은 캘리브레이션 기준점 3개 이상으로 최소자승 적합합니다([`fitPlanToWorld`](src/lib/answer-key.ts)).
3. **같은 색끼리 최적 매칭** — 색상별로 "참가자 마커 ↔ 정답 스톤" 거리 행렬을 만들고, **거리 합이 최소가 되는 조합**으로 짝짓습니다([`bestMatchingDistances`](src/lib/answer-key.ts)). 색상당 3개 이하라 순열 전수로 최적해를 구하므로, 같은 색 안에서 위치를 서로 바꿔 찍어도 가장 가까운 정답에 매칭됩니다.
4. **거리 기준 정답 판정** — 매칭된 두 점의 유클리드 거리(미터)가 임계값(`DEFAULT_THRESHOLD_METERS`, 기본 **3m**) 이내면 정답입니다. 결과로 임계 내 비율(정답률)과 평균 오차(미터)를 색상별·전체로 계산합니다([`ScoreResult`](src/lib/answer-key.ts)).

즉 정확히 같은 좌표일 필요는 없고, 색상 단위로 최적 매칭한 뒤 거리 임계값으로 판정합니다.

> **주의: 채점은 평면도 캘리브레이션에 전적으로 의존합니다.** `plan→world` 변환이 해당 평면도(예: FP2)에 맞게 설정돼 있지 않으면 거리가 통째로 어긋나 정답률이 왜곡됩니다. 평면도나 방 배치를 바꾸면 캘리브레이션 기준점을 반드시 다시 확인하세요. 임계값(기본 3m)도 연구 설계에 맞게 `DEFAULT_THRESHOLD_METERS`에서 조정할 수 있습니다.
...
위치 응답 채점은 **색상별 최적 매칭 + 거리 임계값(기본 3m)** 방식으로 이미 구현되어 있습니다. 자세한 규칙은 위 [위치 응답 채점 방식](#위치-응답-채점-방식) 섹션을 참고하세요.
